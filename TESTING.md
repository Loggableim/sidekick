...
EXPECT:
  ...
  - Sidebar footer shows a single "Sidekick" control-center button
...
EXPECT:
  ...
  - After a few seconds, Sidekick responds
...
EXPECT:
  ...
  - Within 10-30 seconds, Sidekick responds with a three-word greeting
...
EXPECT:
  ...
  - Sidekick responds describing or reading the file content
FAIL: Upload fails, file badge never appears, Sidekick does not mention the file.
...
STEPS:
  1. Click the "Sidekick" button in the sidebar footer
...
4. For T7.x (approval tests): the agent running inside Sidekick needs to detect a
   dangerous command. Ask Sidekick to "run: rm -rf /tmp/test_hermes_approval" and watch
...
EXPECT:
  - A small clipboard icon appears in the message header row (right side of "You" or "Sidekick")
...
EXPECT:
  - "You" and "Sidekick" labels appear in slightly muted blue/gold (not full-brightness)
...
EXPECT:
  ...
  - Sidekick streams a fresh response
...
EXPECT:
  ...
  - Sidekick streams a new response
...
EXPECT:
  - The "Sidekick" button is visible in the sidebar footer
...
SETUP: Ask Sidekick something that produces a code response (e.g. "Show me a Python hello world").
EXPECT:
...
STEPS:
  1. While Sidekick is responding, type a new message and press Enter
EXPECT:
...
- Click the "Sidekick" button in the sidebar footer. Control Center overlay opens with vertical section tabs on the left.
