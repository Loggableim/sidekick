# Sidekick Agent v0.11.0 (v2026.4.23)
...
- **Shell hooks** — Wire any shell script as a Sidekick lifecycle hook (pre_tool_call, post_tool_call, on_session_start, etc.) without writing a Python plugin. ([#13296](https://github.com/NousResearch/hermes-agent/pull/13296))
...
A full React/Ink rewrite of the interactive CLI — invoked via `sidekick --tui` or `HERMES_TUI=1`. Shipped across ~310 commits to `ui-tui/` and `tui_gateway/`.
...
### Image Generation
- **Multi-model FAL support** with picker in `sidekick tools` ([#11265](https://github.com/NousResearch/hermes-agent/pull/11265))
...
- **`sidekick skills reset`** to un-stick bundled skills ([#11468](https://github.com/NousResearch/hermes-agent/pull/11468))
...
- Fix: enforce `config.yaml` as sole CWD source + deprecate `.env` CWD vars + add `sidekick memory reset` ([#11029](https://github.com/NousResearch/hermes-agent/pull/11029))
...
- **Warn about legacy hermes.service units** during `sidekick update` ([#11918](https://github.com/NousResearch/hermes-agent/pull/11918))
