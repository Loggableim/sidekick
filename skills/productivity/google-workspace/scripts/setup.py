...
# Ensure sibling modules (_sidekick_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _sidekick_home import display_sidekick_home, get_sidekick_home
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_sidekick_home()}/google_token.json")
