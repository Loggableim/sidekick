# Sidekick Agent v0.5.0 (v2026.3.28)
...
- **Nous Portal now supports 400+ models** — The Nous Research inference portal has expanded dramatically, giving Sidekick Agent users access to over 400 models through a single provider endpoint
...
- Removed `/model` slash command from CLI and gateway in favor of `sidekick model` subcommand ([#3080](https://github.com/NousResearch/hermes-agent/pull/3080))
...
- Fix plugin toolsets invisible in `sidekick tools` and standalone processes ([#3457](https://github.com/NousResearch/hermes-agent/pull/3457))
- Consolidate `get_hermes_home()` and `parse_reasoning_effort()` ([#3062](https://github.com/NousResearch/hermes-agent/pull/3062))
- Remove unused Sidekick-native PKCE OAuth flow ([#3107](https://github.com/NousResearch/hermes-agent/pull/3107))
...
- Harden `sidekick update` against diverged history, non-main branches, and gateway edge cases ([#3492](https://github.com/NousResearch/hermes-agent/pull/3492))
...
- **OpenClaw migration v2** — 17 new modules, terminal recap for migrating from OpenClaw to Sidekick ([#2906](https://github.com/NousResearch/hermes-agent/pull/2906))
...
- Fix `sidekick update` PEP 668 "externally-managed-environment" error ([#3099](https://github.com/NousResearch/hermes-agent/pull/3099))
