...
TIPS = [
    ...
    "sidekick chat --source telegram tags the session for filtering in hermes sessions list.",
    ...
    "sidekick mcp serve runs Hermes itself as an MCP server for other agents.",
    ...
    "sidekick backup creates a zip backup of your entire Hermes home directory.",
    ...
    "sidekick gateway install sets up Hermes as a system service (systemd/launchd).",
    ...
    "sidekick acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    'sidekick dashboard --tui embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'sidekick chat --ignore-user-config skips ~/.hermes/config.yaml — reproducible bug reports and CI runs.',
    "sidekick chat --source tool tags programmatic chats so they don't clutter hermes sessions list.",
    'sidekick dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'sidekick sessions rename <ID> "new title" renames any past session; hermes sessions delete <ID> removes one.',
    ...
    'sidekick status --deep runs the full health sweep across every component; plain hermes status is the quick view.',
