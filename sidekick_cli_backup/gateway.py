...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from sidekick_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_list() -> None:
    ...
    """
    try:
        from sidekick_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from sidekick_constants import is_container, is_termux, is_wsl
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from sidekick_constants import get_hermes_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
def _setup_whatsapp():
    ...
    from sidekick_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from sidekick_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    """
    from sidekick_cli import setup as _s
    return {
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info("  For persistence:   tmux new -s hermes 'sidekick gateway run'")
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'sidekick gateway run'")
                print()
            ...
            print("  tmux new -s hermes 'sidekick gateway run'         # persistent via tmux")
