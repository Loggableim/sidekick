...
Sidekick Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
| Command | Description |
|---------|-------------|
| `sidekick checkpoints` | Show total size, project count, per-project breakdown |
| `sidekick checkpoints status` | Same as bare `checkpoints` |
| `sidekick checkpoints list` | Alias for `status` |
| `sidekick checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `sidekick checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `sidekick checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Sidekick detects when tools are about to **modify files** in your working tree.
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `sidekick checkpoints prune` manually.
...
Sidekick responds with a formatted list showing change statistics:
...
- **Directory scope** — Sidekick skips overly broad directories (root `/`, home `$HOME`).
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `sidekick checkpoints status` / `prune` / `clear` instead.
...
- **Enable checkpoints only when you need them** — `sidekick chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `sidekick checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Sidekick session in its own worktree/branch, with checkpoints as an extra layer.
