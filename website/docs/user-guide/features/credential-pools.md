...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Sidekick automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Sidekick auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:
...
Run `sidekick auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `sidekick auth` | Interactive pool management wizard |
| `sidekick auth list` | Show all pools and credentials |
| `sidekick auth list <provider>` | Show a specific provider's pool |
| `sidekick auth add <provider>` | Add a credential (prompts for type and key) |
| `sidekick auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `sidekick auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `sidekick auth remove <provider> <index>` | Remove credential by 1-based index |
| `sidekick auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `sidekick auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `sidekick model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.
...
Sidekick automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Sidekick PKCE OAuth | `~/.hermes/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `sidekick auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `sidekick auth add`) are never auto-pruned.
