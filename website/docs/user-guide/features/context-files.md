...
Sidekick Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Sidekick instance and is loaded from `HERMES_HOME` only.
...
| **SOUL.md** | Global personality and tone customization for this Sidekick instance | `HERMES_HOME/SOUL.md` only |
...
- `~/.hermes/SOUL.md`
- or `$HERMES_HOME/SOUL.md` if you run Sidekick with a custom home directory

Important details:

- Sidekick seeds a default `SOUL.md` automatically if one does not exist yet
- Sidekick loads `SOUL.md` only from `HERMES_HOME`
- Sidekick does not probe the working directory for `SOUL.md`
...
Sidekick is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.hermes.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Sidekick.
