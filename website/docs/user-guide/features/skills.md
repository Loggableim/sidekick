...
You can also point Sidekick at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Sidekick to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.
...
When a missing value is encountered, Sidekick asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `sidekick setup` or `~/.hermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `sidekick config migrate` prompts for unconfigured settings, and `sidekick config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
If you maintain skills outside of Sidekick — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Sidekick to scan those directories too.
...
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Sidekick ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Sidekick. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `sidekick skills search <query> --source skills-sh`. Sidekick resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Sidekick currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Sidekick repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Sidekick can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
Sidekick can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
- Sidekick source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Sidekick supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Sidekick source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Sidekick can search and convert agent entries from LobeHub's public catalog into installable Sidekick skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Sidekick source id: `lobehub`

#### 8. Direct URL (`url`)

Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Sidekick fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Sidekick source id: `url`
...
Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `sidekick skills update` re-fetches from the same URL automatically when you want to refresh.
...
`sidekick skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Sidekick | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Sidekick users add with `sidekick skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Sidekick discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Sidekick user can subscribe and install:
...
The `sidekick skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `sidekick skills tap list` shows the effective path per tap.
...
## Bundled skill updates (`sidekick skills reset`)

Sidekick ships with a set of bundled skills in `skills/` inside the repo. On install and on every `sidekick update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Sidekick recomputes the hash of your local copy and compares it to the origin hash:
...
`sidekick skills reset` is the escape hatch:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `sidekick -p coder skills reset <name>` only affects that profile.
:::
