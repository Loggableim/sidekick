...
Sidekick Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `sidekick computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `sidekick tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Sidekick refreshes the binary in two
places so you don't get stuck on a stale release:

- **`sidekick update`** — when you update Sidekick itself, if `cua-driver` is
...
- **`sidekick computer-use install --upgrade`** — manual force-refresh.
...
`sidekick computer-use status` shows the installed version next to the
binary path.
...
Sidekick applies multi-layer guardrails:
...
Screenshots are expensive. Sidekick applies four layers of optimisation:
...
`sidekick computer-use install` to fetch the cua-driver binary, or run
`sidekick tools` and enable the Computer Use toolset.
