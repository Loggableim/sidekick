...
Sidekick Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Sidekick can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`sidekick fallback` reuses the provider picker from `sidekick model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.
...
`fallback_model` (singular) is the legacy single-fallback key — Sidekick still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `sidekick fallback` writes to this key. When both are set, Sidekick merges them with `fallback_providers` taking priority.
:::
...
| Nous Portal | `nous` | `sidekick auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `sidekick model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `sidekick model` (Google OAuth; optional: `HERMES_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `sidekick model` (Qwen Portal OAuth; optional: `HERMES_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `sidekick model` (MiniMax portal OAuth) |
...
Sidekick uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `sidekick kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Sidekick tries providers in order until one works:
...
If the resolved provider fails at call time, Sidekick also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, Sidekick also supports:
...
| `"codex"` | Force Codex OAuth | `sidekick model` → Codex |
...
`base_url` takes precedence over `provider`. Sidekick uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Sidekick drops middle conversation turns without generating a summary rather than failing the session.
