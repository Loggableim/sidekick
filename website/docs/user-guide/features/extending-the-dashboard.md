...
description: "Build themes and plugins for the Sidekick web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Sidekick web dashboard (`sidekick dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
|-------|---------|------------|--------|
| **Sidekick Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Sidekick Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Sidekick Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
// Sidekick API client + raw fetcher
...
#### Calling built-in Sidekick endpoints
...
| `header-left` | Before the Sidekick brand in the top bar. |
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Sidekick Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
2. Restart `sidekick dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
