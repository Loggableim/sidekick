...
sidebar_position: 12
title: "Google Chat"
description: "Set up Sidekick Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Sidekick Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Sidekick
...
download the file. Save it somewhere only Sidekick can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Sidekick calls
...
- **App name**: whatever you want users to see ("Sidekick" is reasonable).
...
`ADDED_TO_SPACE` event that Sidekick uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Sidekick
...
Install the dependencies the Google Chat adapter needs (no Sidekick extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Sidekick is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Sidekick detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Sidekick session.
...
3. Download the JSON. Move it onto the host that runs Sidekick.
4. On the host, register the client with Sidekick:
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Sidekick isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `sidekick gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Sidekick will only attach the SA bearer
