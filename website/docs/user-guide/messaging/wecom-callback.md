...
Connect Sidekick to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Sidekick supports two WeCom integration modes:
...
3. Sidekick decrypts the message, queues it for the agent
...
(Use `sidekick gateway start` only after `sidekick gateway install` has registered the systemd/launchd service.)
