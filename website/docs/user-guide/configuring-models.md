...
Sidekick uses two kinds of model slots:
...
![Models page overview](/img/docs/dashboard-models/overview.svg)
...
![Model picker dialog](/img/docs/dashboard-models/picker-dialog.svg)
...
- **Right** — the curated model list for the selected provider. These are the agentic models Sidekick recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Sidekick writes it to `~/.hermes/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.

## Setting auxiliary models

Click **Show auxiliary** to reveal the eight task slots:

![Auxiliary panel expanded](/img/docs/dashboard-models/auxiliary-expanded.svg)

Every auxiliary task defaults to `auto` — meaning Sidekick uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `sidekick skills search` uses this. Usually fine at `auto`. |
...
![Use as dropdown](/img/docs/dashboard-models/use-as-dropdown.svg)
...
When you save via the dashboard, Sidekick writes to `~/.hermes/config.yaml`:
...
`provider: auto` with `model: ''` tells Sidekick to use the main model for that task.

## When does it take effect?

- **CLI** (`sidekick chat`): next `sidekick chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`sidekick gateway restart`) if you want to force all sessions to pick up the change.
...
Sidekick lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `sidekick setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Sidekick switched providers on me
...
Inside any `sidekick chat` session:
...
### `sidekick model` subcommand

```bash
hermes model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`sidekick model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.hermes/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `sidekick config get model` and `sidekick status`.
...
curl -H "X-Sidekick-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Sidekick-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Sidekick-Session-Token: $TOKEN" \
