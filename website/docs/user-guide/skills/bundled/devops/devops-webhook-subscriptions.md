...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Sidekick agent runs by POSTing events to a URL.
...
All management is via the `sidekick webhook` CLI command:
...
1. `sidekick webhook subscribe` writes to `~/.hermes/webhook_subscriptions.json`
...
4. **Signature mismatch?** Verify the secret in your service matches the one from `sidekick webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `sidekick webhook test <name>` to verify the route works.
