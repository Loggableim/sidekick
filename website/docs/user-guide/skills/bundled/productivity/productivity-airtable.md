...
4. Store the token in `~/.hermes/.env` (or via `sidekick setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Sidekick. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
