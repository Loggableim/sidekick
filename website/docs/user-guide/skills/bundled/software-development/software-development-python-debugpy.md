...
For long-lived processes: Sidekick gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Sidekick you have two practical options:
...
## Debugging Sidekick-specific Processes
...
### `tui_gateway` subprocess (spawned by `sidekick --tui`)
...
debugpy.wait_for_client()
```
Start `sidekick --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Sidekick subagents, debug one process at a time.
