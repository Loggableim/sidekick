...
| | |
|---|---|
| Source | Optional — install with `sidekick skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Sidekick Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Sidekick terminal calls (desktop app flow only)
...
### Service Account (recommended for Sidekick)
...
## Sidekick Execution Pattern (desktop app flow)

Sidekick terminal commands are non-interactive by default and can lose auth context between calls.
