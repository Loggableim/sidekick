---
title: "Sidekick Atropos Environments — Build, test, and debug Sidekick Agent RL environments for Atropos training"
sidebar_label: "Sidekick Atropos Environments"
description: "Build, test, and debug Sidekick Agent RL environments for Atropos training"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Sidekick Atropos Environments

Build, test, and debug Sidekick Agent RL environments for Atropos training. Covers the SidekickAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the hermes-agent repo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sidekick skills install official/mlops/hermes-atropos-environments` |
...
# Sidekick Agent Atropos Environments
...
Atropos BaseEnv (atroposlib/envs/base.py)
    └── SidekickAgentBaseEnv (environments/hermes_base_env.py)
...
Sidekick environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.
...
| `environments/agent_loop.py` | `SidekickAgentLoop` + `AgentResult` dataclass |
...
async def evaluate(self, *args, **kwargs) -> None:
    import time, uuid
    from environments.agent_loop import SidekickAgentLoop
...
2. **evaluate() must use SidekickAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of hermes-agent benchmarks is agentic evaluation with tool use.
