---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Sidekick Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Sidekick Agent"
---
...
Migrate a user's OpenClaw customization footprint into Sidekick Agent. Imports Sidekick-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sidekick skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Sidekick Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Sidekick`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Sidekick Migration

Use this skill when a user wants to move their OpenClaw setup into Sidekick Agent with minimal manual cleanup.
...
**First-time setup:** The `sidekick setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Sidekick home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Sidekick memory entries
- merge OpenClaw command approval patterns into Sidekick `command_allowlist`
- migrate Sidekick-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Sidekick workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Sidekick destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Sidekick-compatible secrets, currently:
...
Sidekick CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Sidekick. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Sidekick workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Sidekick skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Sidekick unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Sidekick targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Sidekick destination. Unsupported auth blobs must still be reported as skipped.
...
- Sidekick persona state imported
- Sidekick memory files populated with converted OpenClaw knowledge
