...
description: "Configure and use Honcho memory with Sidekick -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Sidekick -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sidekick skills install official/autonomous-ai-agents/honcho` |
| Path | `optional-skills/autonomous-ai-agents/honcho` |
| Version | `2.0.0` |
| Author | Sidekick Agent |
| License | MIT |
...
# Honcho Memory for Sidekick

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Sidekick profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Sidekick profile's AI peer
...
Honcho models conversations as interactions between **peers**. Sidekick creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Sidekick instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Sidekick run |
| `global` | Single session across all directories |

Manual override: `sidekick honcho map my-project-name`
...
Each Sidekick profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Sidekick when Honcho memory is active.
...
Run `sidekick honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `sidekick honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sidekick profile create <name> --clone`. For existing profiles: `sidekick honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sidekick honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sidekick honcho status` | Show resolved config, connection test, peer info for active profile |
| `sidekick honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sidekick honcho disable` | Disable Honcho for the active profile |
| `sidekick honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sidekick honcho peers` | Show peer identities across all profiles |
| `sidekick honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sidekick honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sidekick honcho sessions` | List known directory-to-session-name mappings |
| `sidekick honcho map <name>` | Map current working directory to a Honcho session name |
| `sidekick honcho identity` | Seed AI peer identity or show both peer representations |
| `sidekick honcho sync` | Create host blocks for all Sidekick profiles that don't have one yet |
| `sidekick honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Sidekick + Honcho |
| `sidekick memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sidekick memory status` | Show active memory provider and config |
| `sidekick memory off` | Disable external memory provider |
