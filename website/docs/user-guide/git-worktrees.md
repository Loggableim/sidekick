...
title: "Git Worktrees"
description: "Run multiple Sidekick agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Sidekick Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Sidekick so each session has a clean, isolated working directory.

## Why Use Worktrees with Sidekick?

Sidekick treats the **current working directory** as the project root:

- CLI: the directory where you run `hermes` or `sidekick chat`
...
Now you can `cd` into the new worktree and run Sidekick there:

```bash
cd ../repo-feature

# Start Sidekick in the worktree
hermes
```
...
- Sidekick checkpoint data under `~/.hermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Sidekick experiment**
...
- **Avoid running Sidekick from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sidekick -w` (Automatic Worktree Mode)

Sidekick has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
For parallel agents, open multiple terminals and run `sidekick -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Sidekick session its own clean checkout.
