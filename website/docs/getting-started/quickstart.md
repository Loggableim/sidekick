...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Sidekick Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Sidekick setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Sidekick Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Sidekick Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Sidekick for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Sidekick working on my machine | `sidekick setup` | Run a real chat and verify it responds |
| I already know my provider | `sidekick model` | Save the config, then start chatting |
| I want a bot or always-on setup | `sidekick gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `sidekick model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `sidekick model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Sidekick cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Sidekick Agent
...
The single most important setup step. Use `sidekick model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `sidekick model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `sidekick model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `sidekick model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `sidekick model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `sidekick model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `sidekick model` (requires `copilot` CLI + `copilot login`) |
...
Sidekick Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `sidekick model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Sidekick separates secrets from normal config:
...
Sidekick ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `hermes` vs `sidekick --tui`.
:::
...
- Sidekick replies without error
...
- `sidekick tools` — tune tool access per platform
- `sidekick skills` — browse and install reusable workflows
...
```bash
# From the Sidekick install directory (the curl installer placed it at
...
|---|---|---|
| Sidekick opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `sidekick model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `sidekick gateway setup` and check `sidekick gateway status` |
| `sidekick --continue` can't find old session | Switched profiles or session never saved | Check `sidekick sessions list` and confirm you're in the right profile |
...
| `sidekick doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `sidekick model`
3. `sidekick setup`
4. `sidekick sessions list`
5. `sidekick --continue`
6. `sidekick gateway status`
...
| `sidekick tools` | Configure which tools are enabled per platform |
| `sidekick setup` | Full setup wizard (configures everything at once) |
| `sidekick doctor` | Diagnose issues |
| `sidekick update` | Update to latest version |
| `sidekick gateway` | Start the messaging gateway |
| `sidekick --continue` | Resume last session |
