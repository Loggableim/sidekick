...
description: "Install and deploy Sidekick Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Sidekick Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`sidekick setup`, `sidekick gateway install`, config editing) works identically to the standard install.
...
After `nix profile install`, `hermes`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `sidekick setup` walks you through provider selection, `sidekick gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.hermes/`.
...
- The routing is transparent: `sidekick chat`, `sidekick sessions list`, `sidekick version`, etc. all exec into the container under the hood
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Sidekick defaults to OpenRouter.
:::
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Sidekick reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Sidekick looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Sidekick loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Sidekick implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Sidekick prints the authorization URL to stdout/logs instead of opening a browser.
...
| Blocked command | Why |
|---|---|
| `sidekick setup` | Config is declarative — edit `settings` in your Nix config |
| `sidekick config edit` | Config is generated from `settings` |
| `sidekick config set <key> <value>` | Config is generated from `settings` |
| `sidekick gateway install` | The systemd service is managed by NixOS |
| `sidekick gateway uninstall` | The systemd service is managed by NixOS |
...
The NixOS module supports declarative plugin installation — no imperative `sidekick plugins install` needed.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Sidekick discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `hermes-agent` binaries exist and `sidekick version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `sidekick --help` exposes `gateway` and `config` subcommands |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `sidekick gateway` |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Sidekick binary + all Nix deps |
...
| `sidekick version` shows old version | Container not restarted | `systemctl restart hermes-agent` |
