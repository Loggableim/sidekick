...
sidebar_position: 2
title: "Installation"
description: "Install Sidekick Agent on Linux, macOS, WSL2, native Windows (early beta), or Android via Termux"
---

# Installation

Get Sidekick Agent up and running in under two minutes with the one-line installer.
...
Sidekick now ships a Termux-aware installer path too:
...
- **CLI (`sidekick chat`, `sidekick setup`, `sidekick gateway`, …)** — native, uses your default terminal
...
| `API key not set` | Run `sidekick model` to configure your provider, or `sidekick config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `sidekick config check` then `sidekick config migrate` |

For more diagnostics, run `sidekick doctor` — it will tell you exactly what's missing and how to fix it.
