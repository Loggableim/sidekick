...
The ACP adapter wraps Sidekick' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- `allow_once` -> Sidekick `once`
- `allow_always` -> Sidekick `always`
- reject options -> Sidekick `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Sidekick tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Sidekick' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Sidekick provider/credentials.
...
- `hermes_cli/main.py` — `sidekick acp` CLI subcommand
