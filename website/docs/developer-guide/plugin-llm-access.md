...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Sidekick already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Sidekick provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Sidekick provider
        # (must be one Sidekick already knows about — same names as
        # `sidekick model` and config.yaml model.provider).
        ...
        # plugin sends — Sidekick does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Sidekick' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Sidekick subsystem:
