...
description: "How Sidekick stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `sidekick cron` subcommands |
...
In CLI mode, cron jobs only fire when `sidekick cron` commands are run or during active CLI sessions.
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `sidekick cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `sidekick cron` CLI provides direct job management:
