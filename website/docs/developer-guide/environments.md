...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Sidekick-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Sidekick Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Sidekick/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- SidekickAgentBaseEnv
    SidekickAgentBaseEnv <|-- TerminalTestEnv
    SidekickAgentBaseEnv <|-- SidekickSweEnv
    SidekickAgentBaseEnv <|-- TerminalBench2EvalEnv
...
- **Agent loop integration** — `collect_trajectory()` runs `SidekickAgentLoop` and scores the result
...
Your environment inherits from `SidekickAgentBaseEnv` and implements five methods:
...
`SidekickAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as hermes-agent's main loop:
...
```python
from environments.hermes_base_env import SidekickAgentBaseEnv, SidekickAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(SidekickAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(SidekickAgentBaseEnv):
    name = "my-env"
...
```
environments/
├── hermes_base_env.py          # Abstract base class (SidekickAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (SidekickAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── hermes_parser.py        # Sidekick/ChatML <tool_call> format
