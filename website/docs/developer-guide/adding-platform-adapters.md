...
This guide covers adding a new messaging platform to the Sidekick gateway. A platform adapter connects Sidekick to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core Sidekick code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `sidekick config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
def register(ctx):
    """Plugin entry point — called by the Sidekick plugin system."""
...
| `sidekick config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `sidekick status` | Shows plugin platforms with `(plugin)` tag |
| `sidekick gateway setup` | Plugin platforms appear in setup menu |
| `sidekick tools` / `sidekick skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.hermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `sidekick gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `sidekick cron run` separate from `sidekick gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `sidekick config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Sidekick Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Sidekick core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
