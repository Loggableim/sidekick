...
description: "How Sidekick builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Sidekick deliberately separates:
...
You are Sidekick, an AI assistant created by Nous Research.
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | Sidekick-native project config |
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Sidekick already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Sidekick.
...
- if you want to change how Sidekick assembles prompts for everyone, change Python and treat it as a code contribution
