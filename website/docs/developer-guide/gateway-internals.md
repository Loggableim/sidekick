...
The messaging gateway is the long-running process that connects Sidekick to 20+ external messaging platforms through a unified architecture.
...
- `sidekick gateway start` / `sidekick gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.hermes/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `sidekick gateway stop` stops only the current profile's gateway. `sidekick gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
