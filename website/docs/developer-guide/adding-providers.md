...
description: "How to add a new inference provider to Sidekick Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Sidekick can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `sidekick model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `sidekick setup`.
:::
...
7. `sidekick model` menu includes the provider
8. `sidekick setup` wizard delegates to `main.py` automatically
...
- Provider-specific `sidekick model` menu entries with bespoke auth flows
...
- What env vars should Sidekick check, and in what priority order?
...
Be careful with API-key precedence. Sidekick already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `sidekick model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `sidekick model` and `sidekick setup` automatically.
:::
...
When you add a native provider, double-check that Sidekick is only sending fields that provider actually understands.
...
### 7. Updating `sidekick model` but not `sidekick setup`
