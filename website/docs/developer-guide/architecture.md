...
sidebar_position: 1
title: "Architecture"
description: "Sidekick Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Sidekick Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # SidekickCLI — interactive terminal UI (large file)
...
```text
User input → SidekickCLI.process_input()
...
Exposes Sidekick as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`sidekick -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
