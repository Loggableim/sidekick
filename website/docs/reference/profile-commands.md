...
This page covers all commands related to [Sidekick profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `sidekick profile` without a subcommand shows help.
...
This shows the profile's Sidekick home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
## `sidekick profile alias`

```bash
hermes profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Sidekick installation.
...
:::info
`sidekick profile export` / `import` are still the right commands for
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `sidekick -p telemetry`). |
...
Sidekick version, author, env var requirements, the source URL/path, and
...
`sidekick profile list` also shows the distribution name and version in a
`Distribution` column, and `sidekick profile show <name>` / `delete <name>`
...
Sidekick version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Sidekick can clone from.
3. Tell recipients to run `sidekick profile install <your-repo-url>`.
...
Global flag to run any Sidekick command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
- `sidekick profile <TAB>` — subcommands (list, use, create, etc.)
- `sidekick profile use <TAB>` — profile names
- `sidekick -p <TAB>` — profile names
