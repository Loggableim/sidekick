...
description: "Frequently asked questions and solutions to common issues with Sidekick Agent"
---
...
### What LLM providers work with Sidekick?

Sidekick Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Nous Research's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `sidekick login anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `sidekick model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Sidekick Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Sidekick from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash
```

### I run Sidekick in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Sidekick inside WSL2
...
- let Sidekick use the resulting MCP browser tools

This is more reliable than trying to force Sidekick core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Sidekick](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
...
Yes — Sidekick now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Sidekick Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.hermes/`.

### Can I use it offline / with local models?

Yes. Run `sidekick model`, select **Custom endpoint**, and enter your server's URL:
...
Sidekick persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Sidekick — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Sidekick auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Sidekick Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Sidekick Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Sidekick programmatically:
...
**Cause:** Sidekick requires Python 3.11 or newer.
...
**Cause:** Sidekick builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Sidekick runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Sidekick auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hermes/config.yaml`:
...
**Solution:** Exit your session and use `sidekick model` from your terminal to add new providers:

```bash
# Exit the Sidekick chat session first (Ctrl+C or /quit)
...
After adding a new provider via `sidekick model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `sidekick model` (from terminal) |
| Enter/change API keys | `sidekick model` (from terminal) |
...
- Using `sidekick chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Sidekick detected the wrong context length for your model.
...
If this happens on the first long conversation, Sidekick may have the wrong context length for your model. Check what it detected:
...
**Cause:** Sidekick detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Sidekick never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `sidekick chat`
...
**Solution:**
- Verify your bot token is valid with `sidekick gateway setup`
...
#### WSL: Gateway keeps disconnecting or `sidekick gateway start` fails
...
**Solution:** The gateway captures your shell PATH when you run `sidekick gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:
...
**Solution:**
- Try a faster/smaller model: `sidekick chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `sidekick chat -t "terminal"`
...
# Restart Sidekick or reload MCP after config changes
hermes chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Sidekick](/docs/guides/use-mcp-with-hermes)
...
:::warning
If an MCP server crashes mid-request, Sidekick will report a timeout. Check the server's own logs (not just Sidekick logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `sidekick profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `sidekick update`?

`sidekick update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `sidekick update` once — it covers every profile on the machine.
...
**Solution: Delegation config.** Sidekick can route subagents to a different model automatically. Set this in `~/.hermes/config.yaml`:
...
Now when you tell Sidekick "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Sidekick do this?

**Current limitation:** Sidekick profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Sidekick reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `sidekick skills config` settings don't seem to take effect.

**Solution:** Use `sidekick skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`sidekick gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Sidekick creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Sidekick to another machine
...
1. Install Sidekick Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash
   ```
...
4. On the new machine, run `sidekick setup` to verify API keys and provider config are working.
...
### `sidekick backup` vs `sidekick profile export`

| Feature | `sidekick backup` | `sidekick profile export` |
| :--- | :--- | :--- |
...
:::tip
`sidekick backup` produces a consistent snapshot even while Sidekick is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Sidekick installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Sidekick-specific issue — it's a shell config permissions problem.
...
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Sidekick version (`sidekick --version`), and the full error message
