...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Sidekick Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`sidekick claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Sidekick. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hermes/` (by default a single restore-point archive is written before apply, under `~/.hermes/backups/pre-migration-*.zip`; restorable with `sidekick import`). |
...
| What | OpenClaw source | Sidekick destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | Sidekick destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Sidekick skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Sidekick destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | Sidekick config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Sidekick config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Sidekick field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | Sidekick `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Sidekick path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Sidekick equivalent)

These are saved to `~/.hermes/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Sidekick |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Sidekick has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `sidekick cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/docs/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `sidekick webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `sidekick honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `sidekick skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Sidekick profiles |
...
4. **Verify API keys** — run `sidekick status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hermes-gateway`

6. **Check session policies** — verify `sidekick config get session_reset` matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `sidekick whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `sidekick claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `sidekick config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
