---
sidebar_position: 7
title: "Use SOUL.md with Sidekick"
description: "How to use SOUL.md to shape Sidekick Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Sidekick

`SOUL.md` is the **primary identity** for your Sidekick instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Sidekick to feel like the same assistant every time you talk to it — or if you want to replace the Sidekick persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Sidekick should be
- what Sidekick should avoid stylistically
- how Sidekick should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Sidekick is and how Sidekick speaks
...
Sidekick now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Sidekick with a custom home directory, it becomes:
...
Sidekick automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Sidekick does not overwrite it
- if the file exists but is empty, Sidekick adds nothing from it to the prompt

## How Sidekick uses it

When Sidekick starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Sidekick feels.
...
Sidekick already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Sidekick should sound.

# Avoid
What Sidekick should not do.

# Defaults
How Sidekick should behave when ambiguity appears.
```
...
Then restart Sidekick or start a new session.
...
4. Talk to Sidekick for a while
...
### I edited SOUL.md but Sidekick still sounds the same
...
### Sidekick is ignoring parts of my SOUL.md
