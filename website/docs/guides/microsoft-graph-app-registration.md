...
4. Fill in:
   - **Name:** `Sidekick Teams Meeting Pipeline` (or any name you'll recognize).
...
```powershell
# Create a policy scoped to the Sidekick app
New-CsApplicationAccessPolicy `
  -Identity "Sidekick-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Sidekick meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Sidekick-Meeting-Pipeline-Policy" `
...
Sidekick ships a Graph auth smoke-test. From your Sidekick install:
...
3. Restart the gateway so the new secret is picked up: `sidekick gateway restart`.
