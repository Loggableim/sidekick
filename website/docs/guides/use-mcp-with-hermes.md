---
sidebar_position: 6
title: "Use MCP with Sidekick"
description: "A practical guide to connecting MCP servers to Sidekick Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Sidekick

This guide shows how to actually use MCP with Sidekick Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Sidekick tool
- you want Sidekick to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Sidekick to internal APIs, databases, or company systems without modifying Sidekick core

Do not use MCP when:
- a built-in Sidekick tool already solves the job well
...
- Sidekick remains the agent
- MCP servers contribute tools
- Sidekick discovers those tools at startup or reload time
...
If you installed Sidekick with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Sidekick banner/status should show MCP integration when configured
- ask Sidekick what tools it has available
...
## WSL2: bridge Sidekick in WSL to Windows Chrome

This is the practical setup when:

- Sidekick runs inside WSL2
...
In this setup, Sidekick does **not** connect to Chrome directly. Instead:

- Sidekick runs in WSL
- Sidekick starts a local stdio MCP server
...
```text
Sidekick (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Sidekick stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Sidekick core browser transport
...
Then start a fresh Sidekick session or run:
...
Once loaded, Sidekick can use the MCP-prefixed browser tools directly. For example:
...
If Sidekick runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Sidekick from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Sidekick from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Sidekick:
...
2. Sidekick-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Sidekick will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Sidekick to reason over a bounded workspace.
...
Now Sidekick can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Sidekick core.
...
Because Sidekick now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
