...
description: "Diagnose and fix common Sidekick cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`sidekick gateway` for foreground, or `sidekick gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `sidekick cron tick`.
...
If delivery fails, the job still runs — it just won't send anywhere. Check `sidekick cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `sidekick skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `sidekick skills list`.
...
3. The job's `last_run` metadata via `sidekick cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Sidekick config directory). Verify:
```bash
...
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `sidekick skills install <skill-name>`.
...
hermes logs                         # View recent Sidekick logs
...
1. Run the job with `sidekick cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
