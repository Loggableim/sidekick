...
#### Option 1: Sidekick cron (recommended if you already run the Sidekick gateway)

Sidekick ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:
...
```ini
[Unit]
Description=Sidekick Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run Sidekick Teams pipeline subscription maintenance every 12 hours
...
- run `sidekick teams-pipeline maintain-subscriptions --dry-run`
- inspect `sidekick teams-pipeline list --status failed`
...
- run `sidekick teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `sidekick teams-pipeline run`
...
- [ ] `sidekick teams-pipeline validate` returns an OK snapshot
- [ ] `sidekick teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Sidekick cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
