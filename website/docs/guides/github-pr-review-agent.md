...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Sidekick Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Sidekick instantly when PRs are opened or updated.
:::
...
- **Sidekick Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
Make sure Sidekick can access GitHub. Start a chat:
...
Still in the chat, ask Sidekick to review a real PR:
...
A skill gives Sidekick consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.
...
This is what makes the reviewer actually useful. Start a session and teach Sidekick your team's standards:
...
2. Teach Sidekick your conventions via memory (Step 4)
