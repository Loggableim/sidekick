...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sidekick skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Sidekick Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Sidekick ships with a large built-in skill library copied into `~/.sidekick/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sidekick also syncs bundled skills on `sidekick update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sidekick/skills/` tree, it is still shipped with Sidekick; restore it with `sidekick skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "```bash",
        "sidekick skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "sidekick skills install official/blockchain/solana",
        "sidekick skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "sidekick skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
