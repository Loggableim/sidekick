...
~30 Hz (matching xterm key-repeat), summarizes ~/.sidekick/perf.log on exit.
...
Environment overrides:
  HERMES_PERF_LOG     (default ~/.sidekick/perf.log)
...
try:
    ...
    def get_sidekick_home() -> Path:  # type: ignore[misc]
...
DEFAULT_LOG = Path(os.environ.get("HERMES_PERF_LOG", str(get_sidekick_home() / "perf.log")))
DEFAULT_STATE_DB = get_sidekick_home() / "state.db"
