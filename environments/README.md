...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  SidekickAgentBaseEnv    │  hermes_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     SidekickSweEnv    TerminalBench2EvalEnv
...
**SidekickAgentBaseEnv** (`hermes_base_env.py`) extends BaseEnv with hermes-agent specifics:
...
Concrete environments inherit from `SidekickAgentBaseEnv` and implement:
...
`SidekickAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as hermes-agent's `run_agent.py`:
...
Available parsers:
- `hermes` -- Sidekick/ChatML `<tool_call>` XML format
...
├── hermes_base_env.py            # Abstract base (SidekickAgentBaseEnv)
...
2. Create your env file inheriting from `SidekickAgentBaseEnv`
...
2. Inherit from `SidekickAgentBaseEnv`
