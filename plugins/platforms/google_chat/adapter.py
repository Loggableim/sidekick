...
# ``model_tools`` import time, so ``sidekick status``, ``sidekick chat``, etc.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _sidekick_home = _get_sidekick_home()
        except (ModuleNotFoundError, ImportError):
            _sidekick_home = _Path.home() / ".hermes"
        self._thread_count_store = _ThreadCountStore(
            _sidekick_home / "google_chat_thread_counts.json"
        )
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``sidekick setup``.
    ...
    prompt for env vars, persist them to ``~/.sidekick/.env`` so the next
    gateway restart picks them up.
    """
    from sidekick_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print()
    print_success("Google Chat configuration saved to ~/.sidekick/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``sidekick cron`` running as a
    separate process from ``sidekick gateway``).  Without this hook,
