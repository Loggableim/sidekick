...
This plugin ships bundled with Sidekick but is **opt-in** — it only loads when
...
Set these in `~/.hermes/.env` (or via `sidekick tools`):
...
hermes chat -q "hello"              # then check Langfuse for a "Sidekick turn" trace
```
