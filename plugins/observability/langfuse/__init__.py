...
load when listed in ``plugins.enabled`` (via ``sidekick plugins enable
observability/langfuse`` or ``sidekick tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``sidekick tools`` or ~/.sidekick/.env):
