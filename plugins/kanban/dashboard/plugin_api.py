...
This means ``sidekick dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from sidekick_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from sidekick_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    rule definitions.
    """
    from sidekick_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from sidekick_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from sidekick_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``sidekick kanban
...
def list_diagnostics(
    ...
    AND for ``sidekick kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``sidekick kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``sidekick kanban specify <task_id>``.
    ...
    try:
        ...
        from sidekick_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``sidekick kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.sidekick/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.sidekick/profiles/*`` on disk and every
