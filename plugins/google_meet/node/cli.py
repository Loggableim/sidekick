"""`sidekick meet node ...` subcommand tree.

Wired into the existing ``sidekick meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``sidekick meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``sidekick meet node ...``.
