...
│                                                                      │
│   NodeServer (from `sidekick meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `sidekick meet` CLI |
...
| `cli.py` | `sidekick meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `sidekick meet node {run,list,approve,remove,status,ping}` |
