...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Sidekick instances) |
| 2 | `~/.hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Sidekick profile: `hermes` (default) or `hermes.<profile>`.
...
| 4 | `per-session` strategy | Sidekick session ID (`20260415_a3f2b1`) |
...
- **`per-session`** — Sidekick session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Sidekick profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Sidekick profile: `hermes` (default) or `hermes.<profile>` (e.g. `sidekick -p coder` → host key `hermes.coder`).
...
| Command | Description |
|---------|-------------|
| `sidekick honcho setup` | Full interactive setup wizard |
| `sidekick honcho status` | Show resolved config for active profile |
| `sidekick honcho enable` / `disable` | Toggle Honcho for active profile |
| `sidekick honcho mode <mode>` | Change recall or observation mode |
| `sidekick honcho peer --user <name>` | Update user peer name |
| `sidekick honcho peer --ai <name>` | Update AI peer name |
| `sidekick honcho tokens --context <N>` | Set context token budget |
| `sidekick honcho tokens --dialectic <N>` | Set dialectic max chars |
| `sidekick honcho map <name>` | Map current directory to a session name |
| `sidekick honcho sync` | Create host blocks for all Sidekick profiles |
