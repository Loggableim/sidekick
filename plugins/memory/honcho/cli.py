...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `sidekick update` -- no output, no exceptions.
    """
    try:
        from sidekick_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    """
    return get_sidekick_home() / "honcho.json"
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from sidekick_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``sidekick honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``sidekick honcho``.
    """
