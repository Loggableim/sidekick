...
def _teardown_workspace_context() -> None:
    ...
    from api.config import clear_session_dir
    from api.kanban_bridge import clear_workspace_kanban
    from api.space_engine import clear_active_workspace
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/session/context-info":
        ...
        if not sid:
            return bad(handler, "Missing session_id")
        try:
            from api.session_ops import session_context_info
    ...
    if parsed.path == "/api/sessions":
        ...
        try:
            ...
            if workspace_slug:
                try:
                    from api.space_engine import get_workspace
    ...
    if parsed.path == "/api/space/agents":
        ...
        slug = qs.get("slug", [""])[0].strip().lower()
        if slug:
            from api.space_engine import SpaceNotFound
            try:
                from api.space_engine import list_space_agents
    ...
    if parsed.path == "/api/dispatch/active":
        from api.dispatcher import get_active_dispatches as _dispatch_active
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/browser/control":
        try:
            from api.browser_runtime import browser_control
    ...
    if parsed.path == "/api/browser/permission":
        try:
            from api.browser_runtime import (
    ...
    if parsed.path == "/api/browser/agent-control":
        try:
            from api.browser_runtime import browser_agent_control
    ...
    if parsed.path == "/api/browser/action":
        try:
            from api.browser_runtime import browser_action_v1
    ...
    if parsed.path == "/api/space/delete":
        ...
        if not slug:
            return bad(handler, "slug is required")
        from api.space_engine import delete_workspace
    ...
    if parsed.path == "/api/space/agents/list":
        ...
        if not slug:
            return bad(handler, "slug is required")
        from api.space_engine import list_space_agents, SpaceNotFound
        try:
            agents = list_space_agents(slug)
            return j(handler, {"slug": slug, "agents": agents})
        except SpaceNotFound:
    ...
    if parsed.path == "/api/space/agent/create":
        ...
        from api.space_engine import ensure_space_agent
    ...
    if parsed.path == "/api/space/agent/delete":
        ...
        if not slug or not agent:
            return bad(handler, "slug and agent are required")
        from api.space_engine import delete_space_agent
    ...
    if parsed.path == "/api/dispatch/run":
        from api.dispatcher import dispatch_run_once as _dispatch_run
    ...
    # ── Evey Tools API (POST) ──
    if parsed.path.startswith("/api/evey/"):
        from api.evey_tools import handle_evey_post
...
def _handle_agents_get(handler, parsed):
    ...
    if path == "/api/agents/activities":
        ...
        from api.agents import list_activities
    ...
    if path == "/api/agents/stats":
        from api.agents import get_agent_stats
...
def _handle_media(handler, parsed):
    ...
    try:
        from api.space_engine import get_all_spaces
...
def _handle_browser_state(handler, parsed):
    ...
    if not sid:
        return bad(handler, "session_id is required")
    try:
        from api.browser_runtime import browser_state
...
def _handle_browser_permission_status(handler, parsed):
    ...
    if not sid:
        return bad(handler, "session_id is required")
    try:
        from api.browser_runtime import browser_permission_status
...
def _handle_browser_frame(handler, parsed):
    ...
    if not sid:
        return bad(handler, "session_id is required")
    try:
        from api.browser_runtime import browser_frame_bytes
...
def _handle_browser_sse_stream(handler, parsed):
    ...
    if not sid:
        return bad(handler, "session_id is required")
    try:
        from api.browser_runtime import browser_subscribe, browser_unsubscribe
...
def _handle_memory_read(handler):
    try:
        from api.space_engine import resolve_active_space
...
def _async_ollama_title(session_id: str) -> None:
    ...
    try:
        ...
        try:
            from api.models import Session as _Session2
...
def _async_extract_facts(sid: str) -> None:
    ...
    try:
        ...
        from api.models import Session as _Session
        from api.space_engine import get_or_create_space as _get_or_create_space
