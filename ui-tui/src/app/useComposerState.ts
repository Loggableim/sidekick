...
export function useComposerState({
  ...
  const togglePreview = useCallback(() => {
    ...
    // Don't allow toggling INTO preview when busy or input starts with shell command
    if (!previewMode && (uiState.busy || (input.startsWith('!') && !inputBuf.length))) {
      return
    }

    setPreviewMode(p => !p)
  }, [input, inputBuf, previewMode])
  ...
  // Auto-disable preview when the assistant starts streaming
  useEffect(() => {
    if (live.busy && previewMode) {
      setPreviewMode(false)
    }
  }, [live.busy, previewMode])
  ...
  const state = useMemo(
    ...
    [compIdx, compReplace, completions, historyIdx, input, inputBuf, pasteSnips, previewMode, queueEditIdx, queuedDisplay]
  )
