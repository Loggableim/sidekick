...
def _state_file() -> Path:
    return get_sidekick_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.sidekick/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``sidekick update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``sidekick curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `sidekick curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.sidekick/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.sidekick/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.sidekick/logs/curator/``)
    ...
    authored skill data in ``~/.sidekick/skills/``.

    ``ensure_sidekick_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_sidekick_home() / "logs" / "curator"
    try:
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.sidekick/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.sidekick/skills/.archive/` for "
            "safety and can be restored via `sidekick curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.sidekick/skills/.archive/`. "
            "Restore any via `sidekick curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `sidekick curator restore <name>`")
    lines.append("- All archives live under `~/.sidekick/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `sidekick curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `sidekick curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``sidekick model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `sidekick model` → auxiliary
