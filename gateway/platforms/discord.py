...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.sidekick/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_sidekick_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.sidekick/skills/ for new or removed skills")
        ...
        # ── Ticket System ──
        TICKET_CATEGORY_ID = 1460662384963092573  # ✉️ Tickets
        STAFF_ROLE_IDS = [
            1367623607077048509,  # Owner
            1367623607077048508,  # Administrator
            1459245517405159668,  # Head Moderator
            1459245954241663106,  # Moderator
            1459260183548264702,  # Community Manager
            1459247566922449107,  # Support
            1503410786976927904,  # AdminBot
        ]

        @tree.command(name="ticket", description="Erstelle ein Support-Ticket")
        @discord.app_commands.describe(betreff="Kurze Beschreibung deines Anliegens")
        async def slash_ticket(interaction: discord.Interaction, betreff: str = "Support"):
            ...
            try:
                ...
                ticket_name = f"ticket-{user.name.lower().replace(' ', '-')[:20]}"

                # Permission overwrites: user + staff können sehen, @everyone nicht
                overwrites = {
                ...
                for role_id in STAFF_ROLE_IDS:
                    ...
                    if role:
                        overwrites[role] = discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_messages=True, read_message_history=True)
                ...
                embed = discord.Embed(
                    ...
                    description=f"Hallo {user.mention}! Ein Teammitglied wird sich bald um dein Anliegen kümmern.\n\n**Betreff:** {betreff}\n\nZum Schließen des Tickets benutze `/close` in diesem Channel.",
                    color=0x00ff88
                )
                embed.set_footer(text="PawsUnited | Support-Team")
                await ticket_channel.send(content=f"{' '.join(f'<@&{r}>' for r in STAFF_ROLE_IDS[:5])}", embed=embed)

                await interaction.followup.send(f"✅ Ticket erstellt: {ticket_channel.mention}", ephemeral=True)

            except Exception as e:
                logger.error(f"[Ticket] Fehler beim Erstellen: {e}")
                await interaction.followup.send(f"❌ Fehler: {e}", ephemeral=True)

        @tree.command(name="close", description="Schließe das aktuelle Ticket (archiviert den Channel)")
        async def slash_close(interaction: discord.Interaction):
            if not interaction.guild:
                await interaction.response.send_message("❌ Das geht nur auf dem Server.", ephemeral=True)
                return
            channel = interaction.channel
            if not channel or channel.category_id != TICKET_CATEGORY_ID:
                await interaction.response.send_message("❌ Das ist kein Ticket-Channel. Benutze `/close` nur in einem Ticket.", ephemeral=True)
                return
            ...
            try:
                ...
                log_channel = interaction.guild.get_channel(1460661233027121352)  # #ticket-logs
                if log_channel:
                    await log_channel.send(embed=log_embed)

                await channel.send("🔒 **Ticket wird in 5 Sekunden archiviert...**")
                import asyncio
        ...
        try:
            from sidekick_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from sidekick_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from sidekick_cli.commands import discord_skill_commands_by_category
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``sidekick update --gateway``
...
if DISCORD_AVAILABLE:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``sidekick update`` prompts.
