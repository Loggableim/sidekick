...
There are two ways to add a platform to the Sidekick gateway:
...
**zero changes to core Sidekick code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `sidekick gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Sidekick core.
