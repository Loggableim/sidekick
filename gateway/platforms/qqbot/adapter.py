...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.sidekick/.update_response. Set here so the cross-adapter gateway
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.sidekick/.update_response`` for the
          detached ``sidekick update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``sidekick update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `sidekick setup gateway` / `sidekick gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``sidekick update --gateway`` watcher. Button
        ...
        ``~/.sidekick/.update_response`` so the detached update process
        can read it.
        """
