"""ACP session manager — maps ACP sessions to Sidekick AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.sidekick/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Sidekick itself is running in WSL.

    Windows ACP clients can launch ``sidekick acp`` inside WSL while still sending
    ...
    """
    from sidekick_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Sidekick from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["sidekick-acp"]):
...
@dataclass
class SessionState:
"""Tracks per-session state for an ACP-managed Sidekick agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Sidekick AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Sidekick runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.sidekick/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        later (e.g. by the test fixture ``_isolate_sidekick_home``).
        """
