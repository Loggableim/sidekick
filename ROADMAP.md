...
> Web companion to the Nova CLI. Same workflows, browser-native.
>
...
| Surface | Status |
|---|---|
| **Sidekick CLI parity** | ✅ Complete — every CLI workflow has a web equivalent |
...
- [x] Sidekick Control Center modal (centralized actions)
...
| Nova dashboard cross-link | #1459 | Detect a running Nova and surface link in nav |
...
- **Native MCP server expose** — Sidekick as an MCP server for direct agent integration (#733)
- **Teams / agents management panel** — editable names, roles, assignments (#719)
- **Web UI profile model alignment with Sidekick runtime** — design parity (#749)
...
- Plugin marketplace — Nova skills cover this surface
