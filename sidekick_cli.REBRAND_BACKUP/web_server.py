...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from sidekick_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from sidekick_cli.auth import clear_provider_auth
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from sidekick_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from sidekick_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from sidekick_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from sidekick_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from sidekick_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from sidekick_cli import profiles as profiles_mod
    try:
...
async def get_skills():
    ...
    from sidekick_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from sidekick_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from sidekick_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    """
    from sidekick_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _discover_dashboard_plugins() -> list:
    ...
    from sidekick_cli.plugins import get_bundled_plugins_dir
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from sidekick_cli.plugins_cmd import (
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from sidekick_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from sidekick_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sidekick_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sidekick_cli.plugins_cmd import dashboard_remove_user_plugin
