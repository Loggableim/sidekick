...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.sidekick`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.sidekick/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from hermes_constants import get_sidekick_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.sidekick/.env.")
