# Sidekick Agent v0.7.0 (v2026.4.3)
...
- **Camofox Anti-Detection Browser Backend** — New local browser backend using Camoufox for stealth browsing. Persistent sessions with VNC URL discovery for visual debugging, configurable SSRF bypass for local backends, auto-install via `sidekick tools`. ([#4008](https://github.com/NousResearch/hermes-agent/pull/4008), [#4419](https://github.com/NousResearch/hermes-agent/pull/4419), [#4292](https://github.com/NousResearch/hermes-agent/pull/4292))
...
- **API Server Session Continuity & Tool Streaming** — The API server (Open WebUI integration) now streams tool progress events in real-time and supports `X-Sidekick-Session-Id` headers for persistent sessions across requests. Sessions persist to the shared SessionDB. ([#4092](https://github.com/NousResearch/hermes-agent/pull/4092), [#4478](https://github.com/NousResearch/hermes-agent/pull/4478), [#4802](https://github.com/NousResearch/hermes-agent/pull/4802))

- **ACP: Client-Provided MCP Servers** — Editor integrations (VS Code, Zed, JetBrains) can now register their own MCP servers, which Sidekick picks up as additional agent tools. Your editor's MCP ecosystem flows directly into the agent. ([#4705](https://github.com/NousResearch/hermes-agent/pull/4705))
...
- **Unified setup wizard provider selection** with `sidekick model` — single code path for both flows ([#4200](https://github.com/NousResearch/hermes-agent/pull/4200))
...
- **`--max-turns` CLI flag** for `sidekick chat` to limit agent iterations ([#4314](https://github.com/NousResearch/hermes-agent/pull/4314))
...
- **Claude Code credentials gated** behind explicit Sidekick config in wizard trigger ([#4210](https://github.com/NousResearch/hermes-agent/pull/4210))
...
### Update System
- **Fork detection and upstream sync** in `sidekick update` ([#4744](https://github.com/NousResearch/hermes-agent/pull/4744))
...
### Browser
- **Camofox anti-detection browser backend** — local stealth browsing with auto-install via `sidekick tools` ([#4008](https://github.com/NousResearch/hermes-agent/pull/4008))
...
- **Client-provided MCP servers** registered as agent tools — editors pass their MCP servers to Sidekick ([#4705](https://github.com/NousResearch/hermes-agent/pull/4705))
...
- **MCP OAuth server** could block Sidekick startup instead of degrading gracefully ([#4757](https://github.com/NousResearch/hermes-agent/pull/4757), closes [#4462](https://github.com/NousResearch/hermes-agent/issues/4462))
