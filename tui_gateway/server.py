...
load_hermes_dotenv(
    hermes_home=_sidekick_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.sidekick/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_sidekick_home, "logs", "tui_gateway_crash.log")
...
def _save_cfg(cfg: dict):
    ...
    path = _sidekick_home / "config.yaml"
...
def resolve_skin() -> dict:
    try:
        from sidekick_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from sidekick_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from sidekick_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from sidekick_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from sidekick_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from sidekick_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from sidekick_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from sidekick_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from sidekick_cli.config import load_config as _load_full_cfg
...
def _spawn_trees_root():
    ...
    root = get_sidekick_home() / "spawn-trees"
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from sidekick_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from sidekick_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sidekick_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _sidekick_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from sidekick_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_sidekick_home), "display": display_sidekick_home()})
    if key == "full":
...
def _(rid, params: dict) -> dict:
    try:
        from sidekick_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.sidekick/.env`` into the gateway process via
    ...
    """
    try:
        from sidekick_cli.config import reload_env
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`sidekick setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`sidekick gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`sidekick sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`sidekick config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    try:
        from sidekick_cli.commands import resolve_command
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sidekick_cli.plugins import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sidekick_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `sidekick model` and
        ...
        from sidekick_cli.auth import PROVIDER_REGISTRY as _auth_reg
        ...
        for entry in CANONICAL_PROVIDERS:
            if entry.slug in authed_map:
                ...
                if auth_type == "api_key" and key_env:
                    ...
                    warning = f"run `sidekick model` to configure ({auth_type})"
                ordered.append(
                    {
                        "slug": entry.slug,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sidekick_cli.config import is_managed, save_env_value
        from sidekick_cli.model_switch import list_authenticated_providers
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `sidekick model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.sidekick/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sidekick_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from sidekick_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from sidekick_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from sidekick_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from sidekick_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from sidekick_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from sidekick_cli.browser_connect import try_launch_chrome_debug
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_sidekick_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sidekick_cli.tools_config import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from sidekick_cli.skills_hub import do_install
        ...
        if action == "browse":
            from sidekick_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from sidekick_cli.skills_hub import inspect_skill
