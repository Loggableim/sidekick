...
``~/.sidekick/logs/`` (profile-aware via ``get_sidekick_home()``).
...
from sidekick_constants import get_config_path, get_sidekick_home
...
# Used by _ComponentFilter and exposed for ``sidekick logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("sidekick_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    hermes_home
        ...
        ``get_sidekick_home()`` (profile-aware).
    log_level
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from sidekick_cli.config import is_managed
