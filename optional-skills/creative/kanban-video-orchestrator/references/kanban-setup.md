...
the actual `setup.sh` that creates the project workspace, configures Sidekick
...
2. **Create profiles** — `sidekick profile create <name> --clone`
...
6. **Fire the initial kanban task** — `sidekick kanban create` assigned to the director
...
PyYAML must be installed in the user's Python (it ships with most Sidekick
