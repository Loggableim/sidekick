...
Maps each role archetype to the Sidekick skills it should `always_load` and the
...
## Sidekick skills relevant to video production
...
These are **not** Sidekick skills but external CLIs / APIs that profiles invoke.
...
## Built-in Sidekick tools for media review

These are native Sidekick tools — not invoked via terminal but through their own
...
| Anthropic | `ANTHROPIC_API_KEY` | every Sidekick profile (Claude) |
...
When a new Sidekick-public video skill ships:
