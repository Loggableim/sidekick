...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.sidekick/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.sidekick/tts/.",
    },
    "discord-settings": {
...
# become ``~/.sidekick`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.sidekick/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
class Migrator:
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `sidekick claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `sidekick claw cleanup` to fix it.",
            "",
            "## Hermes-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `sidekick claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `sidekick setup` to configure any remaining settings",
            "- Run `sidekick mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `sidekick cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `sidekick cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `sidekick gateway install` if you need the gateway service",
            "- Review `~/.sidekick/config.yaml` for any adjustments",
            "",
        ])
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.sidekick/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.sidekick/config.yaml")
