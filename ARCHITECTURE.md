# Sidekick: Developer and Architecture Guide

> This document is the canonical reference for anyone (human or agent) working on the
> Sidekick. It covers the exact current state of the code, every design decision and
...
The Sidekick is a lightweight web application that gives you a browser-based
interface to the Nova agent that is functionally equivalent to the CLI. It is modeled on
...
Sidekick-level chrome is intentionally consolidated: the sidebar has no dedicated brand header.
Instead, the footer exposes a single "Sidekick" launch button that opens one tabbed
...
- The venv has all Nova agent dependencies (run_agent, tools/*, cron/*)
...
- The server imports Sidekick modules via sys.path.insert(0, parent_dir)
...
Environment variables controlling behavior:
    ...
    HERMES_HOME                    Base directory for Sidekick state (~/.hermes by default)
...
The approval system uses the existing Sidekick gateway module at tools/approval.py.
...
Three-panel layout (in static/index.html):
    ...
    <aside class="sidebar">    Left panel: session list, nav tabs, sidebar-footer Sidekick trigger
...
9.  setBusy(true), setStatus('Sidekick is thinking...')
...
The api/ modules in turn import Sidekick internals:
...
Endpoint that calls Sidekick Python modules:
...
This section is specifically for agents (Sidekick instances, subagents, Codex, etc.) that
