...
## "AIAgent not available -- check that Sidekick agent is on sys.path"
...
**Why it happens.** The WebUI imports the agent class at chat time via `from run_agent import AIAgent`. That import only succeeds if the running Python's `sys.path` contains either the Sidekick agent checkout or a pip-installed copy of the agent. Three common failure modes:

1. **Agent installed but not on `sys.path`.** Most common. The agent is checked out somewhere (e.g. `~/Programmes/sidekick-agent`), the WebUI was launched with a Python that doesn't know about it, and there's no `pip install -e .` linking the two.
...
```bash
# If you have ~/sidekick-agent (the default location):
ls -la ~/sidekick-agent
readlink ~/sidekick-agent          # if it's a symlink, where does it resolve?
ls ~/sidekick-agent/agent/__init__.py 2>&1
```
...
```bash
export HERMES_WEBUI_AGENT_DIR=/absolute/path/to/sidekick-agent
...
```bash
cd /path/to/sidekick-agent        # the directory holding pyproject.toml + the agent/ module
...
```bash
PYTHONPATH=/path/to/sidekick-agent $HERMES_WEBUI_PYTHON -c "from run_agent import AIAgent; print('ok')"
```
