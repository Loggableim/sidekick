...
### Root Cause
The check lives in the **Sidekick agent** upstream repo, not this WebUI:

```
sidekick-agent/tools/browser_tool.py → check_browser_requirements()
```
...
### Fix required
The fix must happen in `sidekick-agent/tools/browser_tool.py`:
