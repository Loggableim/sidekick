...
AlexKucera reported on Discord (2026-04-16) that after ``sidekick update`` pulled
...
Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.sidekick/.env).
"""
...
# Load ~/.sidekick/.env so live runs pick up OPENROUTER_API_KEY without
