...
class TestLoadSessionIdleOverlap:
    """The idle path in loadSession() starts loadDir() before highlightCode(),
    but must not await the workspace tree before completing the chat switch."""
    ...
    def test_idle_path_starts_loaddir_before_highlightcode(self):
        ...
        for pos in positions:
            ...
            if has_highlight and has_loaddir:
                ...
                assert "void loadDir('.')" in block, (
                    "loadDir() should be fire-and-forget so slow workspace "
                    "paths cannot keep session switching stuck."
                )
                assert "await _dirP" not in block, (
                    "loadSession() must not await loadDir(); workspace tree "
                    "loading is secondary to conversation navigation."
                )
                break
