...
TTS provider tools must resolve API keys from ``~/.sidekick/.env`` (via
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.sidekick/.env`` carries the
...
class TestRegressionGuard:
    ...
    ``hermes_cli.config.load_env`` to simulate ``~/.sidekick/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        ...
        """
        import importlib
        import sidekick_cli.config as config_mod
        from tools import tts_tool
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.sidekick/.env carrying the key (load_env returns the dict
        ...
        with patch(
            ...
            # os.environ is empty.
            from sidekick_cli.config import get_env_value as live_get
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.sidekick/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
