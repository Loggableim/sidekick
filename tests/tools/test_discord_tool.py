...
class TestToolsetInclusion:
    def test_discord_tools_in_hermes_discord_toolset(self):
        ...
        assert "discord" in TOOLSETS["sidekick-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["sidekick-discord"]["tools"]
    ...
    def test_discord_tools_not_in_other_toolsets(self):
        ...
        for name, ts in TOOLSETS.items():
            if name in ("sidekick-discord", "sidekick-gateway", "discord", "discord_admin"):
                continue
            tools = ts.get("tools", [])
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["sidekick-discord"], quiet_mode=True)
