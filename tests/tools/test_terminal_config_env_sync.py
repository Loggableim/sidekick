...
dicts) keeps the test independent of the user's ~/.sidekick/config.yaml and
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``sidekick config set foo bar``."""
    from sidekick_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``sidekick config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`sidekick config set terminal.X` doesn't sync these load-bearing "
