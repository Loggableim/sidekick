...
``~/.sidekick/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.sidekick/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.sidekick/.env`` carrying the key while ``os.environ`` does not.
