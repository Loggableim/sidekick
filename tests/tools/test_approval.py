...
class TestTeePattern:
    ...
    def test_tee_hermes_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.sidekick/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_sidekick_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $HERMES_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_sidekick_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$HERMES_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_sidekick_home_env(self):
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.sidekick/hermes-agent && source venv/bin/activate && python -m hermes_cli.main gateway run --replace &disown; echo done"
