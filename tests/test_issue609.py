...
def test_path_outside_boot_default_and_home_is_rejected(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ws_mod.Path, "home", classmethod(lambda cls: fake_home))
    monkeypatch.setattr(ws_mod, "_BOOT_DEFAULT_WORKSPACE", str(boot_default))
...
def test_space_project_dir_is_trusted_without_saved_workspace(monkeypatch, tmp_path):
    """A configured Space project_dir is trusted even when it is not in workspaces.json."""
    import api.space_engine as space_engine
    import api.workspace as ws_mod

    fake_home = tmp_path / "home"
    fake_home.mkdir()
    boot_default = tmp_path / "boot-workspace"
    boot_default.mkdir()
    project_dir = tmp_path / "external-space-project"
    project_dir.mkdir()
    spaces_root = tmp_path / "spaces"
    space_root = spaces_root / "client"
    ...
    monkeypatch.setattr(ws_mod.Path, "home", classmethod(lambda cls: fake_home))
    monkeypatch.setattr(ws_mod, "_BOOT_DEFAULT_WORKSPACE", str(boot_default))
    monkeypatch.setattr(ws_mod, "load_workspaces", lambda: [])
    monkeypatch.setattr(space_engine, "SPACES_ROOT", spaces_root.resolve())
    monkeypatch.setattr(space_engine, "_OLD_ROOT", (tmp_path / "legacy-spaces").resolve())
    space_engine._invalidate_space_cache()

    result = resolve_trusted_workspace(str(project_dir))
    assert result == project_dir.resolve()


def test_space_project_dir_children_are_trusted(monkeypatch, tmp_path):
    """Subdirectories under a configured Space project_dir are also workspace-valid."""
    import api.space_engine as space_engine
    import api.workspace as ws_mod
    ...
    result = resolve_trusted_workspace(str(child))
