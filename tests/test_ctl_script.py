...
def test_start_writes_pid_under_hermes_home_runs_foreground_no_browser_and_logs(tmp_path):
    ...
    try:
        ...
        assert str(hermes_home / "webui").replace("\\", "/") in fake_output.replace("\\", "/")
        status = run_ctl(tmp_path, "status")
        assert status.returncode == 0
        status_out = status.stdout or ""
        assert "PID:" in status_out
        assert "Bound:   0.0.0.0:18991" in status_out
        assert f"Log:     {str(log_file).replace('\\', '/')}" in status_out.replace("\\", "/")
    finally:
