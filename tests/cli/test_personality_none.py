...
class TestGatewayPersonalityNone:
    ...
    async def test_none_clears_ephemeral_prompt(self, tmp_path):
        ...
        with patch("gateway.run._sidekick_home", tmp_path):
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._sidekick_home", tmp_path), \
             patch("hermes_constants.display_sidekick_home", return_value="~/.sidekick/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.sidekick/profiles/coder/config.yaml`"
