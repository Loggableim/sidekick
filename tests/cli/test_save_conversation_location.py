...
file to be discoverable via ``sidekick sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.sidekick/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(hermes_constants, "_sidekick_home_cache"):
        hermes_constants._sidekick_home_cache = None
    return home
...
def test_save_conversation_writes_under_sidekick_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.sidekick/sessions/saved/, not CWD."""
    ...
    # File MUST be under ~/.sidekick/sessions/saved/
