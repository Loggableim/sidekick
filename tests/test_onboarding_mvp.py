...
def clean_hermes_config_files():
    ...
    for rel in ("config.yaml", ".env", "auth.json"):
...
def test_onboarding_status_defaults_incomplete():
    ...
    if data["system"]["config_exists"] is False:
        assert data["system"]["provider_ready"] is False
    assert data["system"]["setup_state"] in {"needs_provider", "provider_incomplete", "agent_unavailable"}
...
def test_onboarding_setup_openrouter_writes_real_config_and_env():
    ...
    cfg_text = pathlib.Path(data["system"]["config_path"]).read_text(encoding="utf-8")
    env_text = pathlib.Path(data["system"]["env_path"]).read_text(encoding="utf-8")
...
def test_onboarding_setup_detects_incomplete_saved_provider():
    ...
    pathlib.Path(status["system"]["env_path"]).unlink(missing_ok=True)
