...
class TestHermesConstantsFallback:
    """Tests for _sidekick_home.py fallback when hermes_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_sidekick_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _sidekick_home.py with hermes_constants blocked."""
        monkeypatch.setitem(sys.modules, "hermes_constants", None)
        spec = importlib.util.spec_from_file_location("_sidekick_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_sidekick_home_env_var(self, monkeypatch, tmp_path):
        ...
        assert module.get_sidekick_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When hermes_constants is missing and HERMES_HOME unset, default to ~/.sidekick."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_sidekick_home() == Path.home() / ".hermes"

    def test_fallback_ignores_empty_sidekick_home(self, monkeypatch):
        """Empty/whitespace HERMES_HOME is treated as unset."""
        monkeypatch.setenv("HERMES_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_sidekick_home() == Path.home() / ".hermes"

    def test_fallback_display_sidekick_home_shortens_path(self, monkeypatch):
        """Fallback display_sidekick_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_sidekick_home() == "~/.sidekick"

    def test_fallback_display_sidekick_home_profile_path(self, monkeypatch):
        """Fallback display_sidekick_home() handles profile paths under ~/."""
        monkeypatch.setenv("HERMES_HOME", str(Path.home() / ".hermes/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_sidekick_home() == "~/.sidekick/profiles/coder"

    def test_fallback_display_sidekick_home_custom_path(self, monkeypatch):
        """Fallback display_sidekick_home() returns full path for non-home locations."""
        monkeypatch.setenv("HERMES_HOME", "/opt/hermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_sidekick_home() == "/opt/hermes-custom"

    def test_delegates_to_hermes_constants_when_available(self):
        """When hermes_constants IS importable, _sidekick_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_sidekick_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import hermes_constants
        assert module.get_sidekick_home is hermes_constants.get_sidekick_home
        assert module.display_sidekick_home is hermes_constants.display_sidekick_home
