...
def test_provider_alias_entry_routes_to_local_openai_compatible_endpoint():
    old_cfg = dict(config.cfg)
    config.cfg.clear()
    config.cfg.update({
        "model": "local-gpu",
    ...
    try:
        model, provider, base_url = config.resolve_model_provider("local-gpu")
        api_key, custom_base_url = config.resolve_custom_provider_connection(provider)
    finally:
        config.cfg.clear()
        config.cfg.update(old_cfg)

    assert model == "Dolphin3.0-Llama3.1-8B-Q4_K_M"
    assert provider == "custom:local-gpu"
    assert base_url == "http://localhost:8080/v1"
    assert api_key == "sk-local"
    assert custom_base_url == "http://localhost:8080/v1"
