...
Patrick-81 reported a symlinked Sidekick agent install that produced a bare
"AIAgent not available -- check that Sidekick agent is on sys.path" error with
...
This test suite locks the diagnostic shape of the new error message:
  ...
  - SIDEKICK_WEBUI_AGENT_DIR is shown if set, "(not set)" otherwise.
...
class TestAIAgentImportErrorDetail:
    ...
    def test_preserves_original_message_for_log_scrapers(self):
        ...
        assert first == "AIAgent not available -- check that Sidekick agent is on sys.path", (
    ...
    def test_shows_agent_dir_env_when_set(self, monkeypatch):
        """If SIDEKICK_WEBUI_AGENT_DIR is set, the diagnostic must show its value
        so the user can confirm whether the override is pointing at the right
        directory.
        """
        helper = _import_helper()
        monkeypatch.setenv("SIDEKICK_WEBUI_AGENT_DIR", "/custom/agent/path")
        out = helper()
        assert "SIDEKICK_WEBUI_AGENT_DIR: /custom/agent/path" in out

    def test_shows_agent_dir_env_unset_marker(self, monkeypatch):
        """If SIDEKICK_WEBUI_AGENT_DIR is NOT set, the diagnostic must say so
        ...
        out = helper()
        assert "SIDEKICK_WEBUI_AGENT_DIR: (not set)" in out
    ...
    def test_lists_sys_path_entries_when_relevant(self, monkeypatch):
        """If sys.path contains entries mentioning sidekick/agent, the diagnostic
    ...
    def test_handles_no_relevant_sys_path_entries(self, monkeypatch):
        """If sys.path has NO sidekick/agent-related entries, the diagnostic must
        ...
        out = helper()
        assert "no entries mention sidekick or agent" in out, (
