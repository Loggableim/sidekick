...
2. **Isolated HERMES_HOME.** HERMES_HOME points to a per-test tempdir so
   code reading ``~/.sidekick/*`` via ``get_sidekick_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_sidekick_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.sidekick/kanban.db instead of the per-test HERMES_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    reads ``~/.sidekick/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.sidekick/*`` via ``get_sidekick_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.sidekick/*`` via
    #    ``Path.home() / ".hermes"`` instead of ``get_sidekick_home()``
    #    is a bug to fix at the callsite.
    fake_sidekick_home = tmp_path / "hermes_test"
    fake_sidekick_home.mkdir()
    (fake_sidekick_home / "sessions").mkdir()
    (fake_sidekick_home / "cron").mkdir()
    (fake_sidekick_home / "memories").mkdir()
    (fake_sidekick_home / "skills").mkdir()
    monkeypatch.setenv("HERMES_HOME", str(fake_sidekick_home))
    ...
    #    ~/.sidekick/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import sidekick_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_sidekick_home(_hermetic_environment):
