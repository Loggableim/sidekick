...
# Windows Python in this environment defaults to cp1252, which breaks the
# repo's UTF-8 static assets during collection on bare Path.read_text()/open().
# Normalize the test harness to UTF-8 when encoding is omitted.
_REAL_PATH_READ_TEXT = pathlib.Path.read_text
_REAL_OPEN = builtins.open

def _utf8_path_read_text(self, encoding=None, errors=None):
    if encoding is None:
        encoding = "utf-8"
    return _REAL_PATH_READ_TEXT(self, encoding=encoding, errors=errors)
...
pathlib.Path.read_text = _utf8_path_read_text
...
def _safe_rmtree(path):
    ...
    def _onerror(func, p, exc_info):
        ...
        if winerror in (2, 5, 145):
            return
        if isinstance(exc, OSError) and os.path.islink(p):
            try:
                os.unlink(p)
                return
            except Exception:
...
def test_server():
    ...
    env.update({
        ...
        "HERMES_WEBUI_LOG_FILE":          str(TEST_STATE_DIR / "logs" / "webui.log"),
