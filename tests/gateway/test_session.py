...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_sidekick_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("hermes_constants.display_sidekick_home", return_value="~/.sidekick/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.sidekick/profiles/coder/cron/output/" in prompt
