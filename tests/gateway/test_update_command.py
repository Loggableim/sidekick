...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _sidekick_home to tmp_path and project_root to a dir without .git
    ...
    async def test_no_hermes_binary(self, tmp_path):
        ...
        assert "sidekick update" in result
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        ...
        with patch("gateway.run._sidekick_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
