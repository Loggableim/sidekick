...
class TestHermesApiServerToolset:
    ...
    def test_toolset_exists(self):
        ts = get_toolset("sidekick-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("sidekick-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("sidekick-api-server")
...
class TestApiServerPlatformConfig:
    def test_platforms_dict_includes_api_server(self):
        from sidekick_cli.tools_config import PLATFORMS
        assert "api_server" in PLATFORMS
        assert PLATFORMS["api_server"]["default_toolset"] == "sidekick-api-server"
