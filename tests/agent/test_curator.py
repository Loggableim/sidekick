...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `sidekick curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    ...
    from sidekick_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `sidekick model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from sidekick_cli.main import _AUX_TASKS
    from sidekick_cli.web_server import _AUX_TASK_SLOTS
