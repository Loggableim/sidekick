...
def test_kanban_assignee_dropdown_uses_select_not_freetext():
    ...
    assert "Nova profile" in hint_block or "data-i18n=\"kanban_assignee_hint\"" in hint_block
...
def test_kanban_ui_parity_polish_css_and_i18n_exist():
    ...
    main_locale_keys = {
        locale
        for locale, body in locale_blocks
        if locale in {"en", "de"} and re.search(r"\bkanban_select_task_hint\s*:", body)
    }
    assert main_locale_keys == {"en", "de"}
...
def test_kanban_review_feedback_static_ui_fixes_exist():
    ...
    assert "kanban_select_task_hint" in PANELS
    assert "Select a task card to view details." in PANELS
