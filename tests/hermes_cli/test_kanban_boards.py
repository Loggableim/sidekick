"""Tests for the multi-board kanban layer (``sidekick kanban boards …``).
...
* CLI surface: ``sidekick kanban boards list/create/switch/rm``.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``sidekick kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
