"""Regression test: ``sidekick mcp add --command`` must not clobber the
...
result, ``sidekick mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`sidekick mcp add foo --url ...` must keep args.command == "mcp".
