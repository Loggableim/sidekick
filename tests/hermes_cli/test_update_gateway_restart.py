...
Ensures ``sidekick update`` correctly detects running gateways managed by
...
class TestServicePidExclusion:
    ...
    where ``sidekick update`` would restart the gateway and immediately kill it.
    """
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``sidekick update --gateway``, the exit code marker must be
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy hermes.service warning printed by `sidekick update`.
    ...
    ``hermes-gateway.service`` after PR #5646. Every ``sidekick update``
    should remind them to run ``sidekick gateway migrate-legacy`` until
    they do.
    """
...
class TestCmdUpdateResetFailedBeforeRestart:
    """`sidekick update` must call `systemctl reset-failed` before every
    ...
    Mirrors the recovery pattern `sidekick gateway restart` (systemd_restart)
