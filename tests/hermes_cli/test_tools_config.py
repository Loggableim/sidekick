...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # auto-recovered by _get_platform_tools so saving via `sidekick tools`
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    ...
    ``sidekick tools``, but hermes-cli stays in the config and re-enables
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`sidekick tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `sidekick tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `sidekick tools`, the toolset appears."""
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `sidekick tools` → "reconfigure existing" shows
