"""Tests for ``sidekick dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``sidekick update``
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`sidekick update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``sidekick dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``sidekick dashboard --stop`` / ``--status`` actually parse."""
