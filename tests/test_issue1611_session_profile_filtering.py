...
def test_static_sessions_js_uses_all_profiles_query_when_toggle_on():
    ...
    assert "if (_showAllProfiles) sessionParams.set('all_profiles', '1')" in src, (
        "Expected fetch path to flip on the toggle state"
    )
    assert "sessionParams.set('workspace', activeSpace)" in src, (
        "Expected /api/sessions fetch to preserve workspace isolation"
    )
    assert "api(sessionsUrl)" in src, (
        "Expected /api/sessions fetch to use the composed query URL"
    )
