...
def test_resolve_model_provider_uses_auth_store_active_provider_when_config_provider_missing(
    tmp_path, monkeypatch
):
    _configure_isolated_sources(tmp_path, monkeypatch, "opencode-go")

    model, provider, base_url = config.resolve_model_provider("glm-5.1")
