...
class TestBootJsThemeColorSync:
    ...
    def test_set_resolved_theme_calls_sync_in_both_branches(self):
        """_setResolvedTheme must update theme-color after the resolved theme changes."""
        src = BOOT.read_text(encoding="utf-8")
        body_start = src.index("function _setResolvedTheme")
        body_end = src.index("// ── Syntax highlighting theme (3 presets) ──", body_start)
        body = src[body_start:body_end]
        assert "document.documentElement.classList.toggle('dark',!!isDark)" in body
        assert "_applySyntaxTheme();" in body
        assert "_syncThemeColorMeta();" in body
