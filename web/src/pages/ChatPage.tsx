...
import { Code2, Copy, PanelRight, X } from "lucide-react";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  // --- Code mode ↔ Chat mode toggle ---------------------------------------
  const MODE_STORAGE_KEY = "hermes-chat-mode";
  const [codeMode, setCodeMode] = useState(() => {
    if (typeof window === "undefined") return false;
    try {
      return localStorage.getItem(MODE_STORAGE_KEY) === "code";
    } catch {
      return false;
    }
  });
  ...
  const handleToggleMode = useCallback(() => {
    setCodeMode((prev) => {
      const next = !prev;
      try {
        localStorage.setItem(MODE_STORAGE_KEY, next ? "code" : "chat");
      } catch {
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        <div className="relative min-h-0 min-w-0 flex-1">
          <SplitPane
            key={`split-${codeMode ? "code" : "chat"}`}
            id={`chat-terminal${codeMode ? "-code" : ""}`}
            defaultOrientation="horizontal"
            defaultSizes={codeMode ? [0.28, 0.72] : [0.35, 0.65]}
            className="min-h-0 min-w-0 flex-1"
          >
            <MessagePanel channel={channel} compact={codeMode} />

            <div
              className={cn(
                "relative flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-lg",
                "p-2 sm:p-3",
              )}
              style={{
          ...
          {/* Mode toggle — floating bottom-left */}
          <button
            type="button"
            ...
            className={cn(
              ...
              "rounded border border-border/20",
              "bg-background-base/40 backdrop-blur-sm",
              "text-midground/40 hover:text-midground hover:bg-background-base/70 hover:border-border/40",
              "transition-all duration-150",
              "px-2 py-1 text-[0.55rem] font-medium uppercase tracking-wider",
              "bottom-2 left-2",
            )}
            aria-label={`Switch to ${codeMode ? "Chat" : "Code"} mode`}
            title={codeMode ? "Switch to Chat Mode" : "Switch to Code Mode"}
          >
            <Code2 className="h-3 w-3 shrink-0" />
            <span>{codeMode ? "Chat" : "Code"}</span>
          </button>
        </div>
