...
export const api = {
  ...
  getGitStatus: () =>
    fetchJSON<GitStatusResponse>("/api/workspace/git-status"),

  // Discord channel management
  getDiscordGuilds: () =>
    fetchJSON<DiscordDiscordGuildsResponse>("/api/discord/guilds"),
  getDiscordChannels: (guildId: string) =>
    fetchJSON<DiscordChannelsResponse>(`/api/discord/guilds/${encodeURIComponent(guildId)}/channels`),
  reorderDiscordChannel: (channelId: string, body: DiscordReorderRequest) =>
    fetchJSON<DiscordReorderResponse>(`/api/discord/channels/${encodeURIComponent(channelId)}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    }),

  // Discord role & member management (Drag & Drop)
  getDiscordRoles: (guildId: string) =>
    fetchJSON<DiscordRolesResponse>(`/api/discord/guilds/${encodeURIComponent(guildId)}/roles`),
  getDiscordMembers: (guildId: string, limit = 200) =>
    fetchJSON<DiscordMembersResponse>(
      `/api/discord/guilds/${encodeURIComponent(guildId)}/members?limit=${limit}`,
    ),
  setDiscordMemberRoles: (memberId: string, body: DiscordMemberRolesRequest) =>
    fetchJSON<DiscordMemberRolesResponse>(
      `/api/discord/members/${encodeURIComponent(memberId)}/roles`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
...
export interface DiscordDiscordGuildsResponse {
  guilds: DiscordGuild[];
}
...
export interface DiscordReorderRequest {
  position?: number;
  parent_id?: string | null;
}

export interface DiscordReorderResponse {
  ok: boolean;
}

// ── Discord Role & Member types (Drag & Drop) ─────────────────────────
...
export interface DiscordRole {
  ...
  position: number;
  member_count: number;
  member_preview: DiscordRoleMember[];
}
...
export interface DiscordMemberRolesRequest {
  guild_id: string;
  add_role_ids?: string[];
  remove_role_ids?: string[];
}

export interface DiscordMemberRolesResponse {
  ok: boolean;
}
