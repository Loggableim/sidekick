# Contributing to Sidekick Agent

Thank you for contributing to Sidekick Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Sidekick to work everywhere.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `sidekick skills browse` (labeled "official") and install it with `sidekick skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `sidekick skills install`.
...
├── cli.py                    # SidekickCLI class — interactive TUI, prompt_toolkit integration
...
The user may skip setup and keep loading the skill. Sidekick only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `sidekick setup` or update `~/.hermes/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Sidekick tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `sidekick --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Sidekick uses a data-driven skin system — no code changes needed to add a new skin.
...
Sidekick runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Sidekick has terminal access. Security matters.
...
- Include: OS, Python version, Sidekick version (`sidekick version`), full error traceback
